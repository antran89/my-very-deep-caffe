...
#ifndef CAFFE_TWOSTREAM_DATA_READER_HPP_
#define CAFFE_TWOSTREAM_DATA_READER_HPP_
 ...
 */
class TwostreamDataReader {
public:
    explicit TwostreamDataReader(const LayerParameter& param);
    ~TwostreamDataReader();
...
protected:
    ...
    protected:
        ...
        void read_one(db::Cursor* cursor, db::Cursor *rgb_cursor, QueuePair* qp);

        const LayerParameter param_;
        BlockingQueue<shared_ptr<QueuePair> > new_queue_pairs_;

        friend class TwostreamDataReader;
    ...
    static map<const string, boost::weak_ptr<TwostreamDataReader::Body> > bodies_;

    DISABLE_COPY_AND_ASSIGN(TwostreamDataReader);
};

}  // namespace caffe

#endif  // CAFFE_TWOSTREAM_DATA_READER_HPP_
