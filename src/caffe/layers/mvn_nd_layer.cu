...
void MVN_NDLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->gpu_data();
  Dtype* top_data = top[0]->mutable_gpu_data();

  // subtract mean
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_, dim_, 1. / dim_, bottom_data,
      sum_multiplier_.gpu_data(), 0., mean_.mutable_gpu_data());  // EX
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_, dim_, 1, -1.,
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, num_, dim_, 1. / dim_, temp_.gpu_data(),
...
template <typename Dtype>
void MVN_NDLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->layer_param_.mvn_param().normalize_variance()) {
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, num_, dim_, 1., bottom_diff,
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, num_, dim_, 1., top_diff,
            sum_multiplier_.gpu_data(), 0., mean_.mutable_gpu_data());
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_, dim_, 1, 1.,
            mean_.gpu_data(), sum_multiplier_.gpu_data(), 1.,
            bottom_diff);

    caffe_gpu_axpby(temp_.count(), Dtype(1), top_diff, Dtype(-1. / dim_),
        bottom_diff);
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, num_, dim_, 1. / dim_, top_diff,
            sum_multiplier_.gpu_data(), 0., mean_.mutable_gpu_data());
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_, dim_, 1, -1.,
            mean_.gpu_data(), sum_multiplier_.gpu_data(), 0.,
            temp_.mutable_gpu_data());
...
INSTANTIATE_LAYER_GPU_FUNCS(MVN_NDLayer);
