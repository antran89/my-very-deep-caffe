...
#include "caffe/layers/video_clip_data_layer.hpp"
#include "caffe/util/benchmark.hpp"

namespace caffe {

template <typename Dtype>
VideoClipDataLayer<Dtype>::VideoClipDataLayer(const LayerParameter& param)
  : BasePrefetchingDataLayer<Dtype>(param),
    reader_(param), num_test_views_(1) {
}

template <typename Dtype>
VideoClipDataLayer<Dtype>::~VideoClipDataLayer() {
  this->StopInternalThread();
}

template <typename Dtype>
void VideoClipDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int batch_size = this->layer_param_.video_data_param().batch_size();
  // Read a data point, and use it to initialize the top blob.
  Datum& datum = *(reader_.full().peek());
  num_test_views_ = this->layer_param_.video_data_param().test_10view_features() ? 10 : 1;
...
template<typename Dtype>
void VideoClipDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
...
INSTANTIATE_CLASS(VideoClipDataLayer);
REGISTER_LAYER_CLASS(VideoClipData);
